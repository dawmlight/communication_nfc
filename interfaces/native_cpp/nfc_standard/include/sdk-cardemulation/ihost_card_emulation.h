...
public:
    ...
    /**
     * brief: set the handler that handles the APDU command
     * parameter: 
     *   handler -- the handler that handles the APDU command
     * return: 0 -- succeeded, not 0 -- error code
     */
    virtual bool SetApduCommandCallback(ApduCommandHandler handler) = 0;
    /**
     * brief: set the handler that handles the deactive events
     * parameter: 
     *   handler -- the handler that handles the deactive events
     * return: 0 -- succeeded, not 0 -- error code
     */
