...
static unsigned char HexCharToByte(char ch) noexcept
{
    if (ch >= '0' && ch <= '9') {
...
struct RoutingEntry {
    ...
    static std::unique_ptr<RoutingEntry> Create(const std::vector<uint8_t>& buf)
    {
        ...
        constexpr size_t entryFieldLenMinValue = 2;  // 条目长度字段最小值
        ...
        if (static_cast<size_t>(buf[posLen]) == entryFieldLenMinValue &&
            !IsPrefixPattern(buf[posHead])) {  // 空 aid 的条目为默认路由，默认路由必须是前缀匹配模式
            return result;
        }
...
public:
    ...
    std::vector<std::vector<RoutingEntry>> getAllEntries(const std::vector<uint8_t>& routingTable)
    {
        ...
        while (offset < routingTable.size()) {
            // [其他数据][包含路由项条目数（1字节）和 aid路由项的数据]技术路由项（2条）和协议路由项（2条）
            // 特殊路由项的开始位置，即技术路由项（2条）和协议路由项（2条）的开始位置
    ...
    // powerState: 0x11（默认路由是host时） or 0x3B(在默认路由是0x82见过)
